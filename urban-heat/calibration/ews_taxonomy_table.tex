...
&
URBADAPT-HEAT treats this class as an intermediate-maturity system: a national heat-warning or heat-action framework exists, but triggers are mainly meteorological, biometeorological or historically calibrated rather than based on real-time mortality surveillance, so the adaptation is an \emph{upgrade} toward a more integrated heat--health system. Setup costs and ramp-up are intended to be intermediate between mature systems and deployment from scratch, with wider efficacy uncertainty. (In the current configuration these countries are still represented with their binary archetype---mature or establish-from-baseline---pending implementation of a distinct intermediate tier.) \textbf{Limitations:} subnational implementation, health-sector coordination, threshold calibration and follow-up actions differ strongly across these countries and are only approximately represented.
&
