...
sensitivity figure -- from the authoritative NB09 export (N=128 Masselot headline).
    ...
    python build_fig_uncertainty.py
...
(median + IQR); (e-g) top-10 PAWN drivers for annual mortality, EWS net avoided
deaths, and the 2050 SVI P90-P10 gap.
"""
...
TGZ = os.path.expanduser(
    "~/Downloads/gmd_nb09_export_2026-09-21/gmd_nb09_four_pilots_complete_v3_616fbc2.tgz") # to change if different 
RUN = "n128_seed42_v3_616fbc2d5131"     # headline Masselot N=128
CITY = "rome"
OUT = os.path.join(os.path.dirname(__file__), "fig_uncertainty.png")
AC, EWS, TREE = "#d1495b", "#1f77b4", "#2a9d3a"

_tmp = tempfile.mkdtemp(prefix="gmd_nb09_fig_")
with tarfile.open(TGZ) as t:
    t.extractall(_tmp)
D = os.path.join(_tmp, "outputs_variants", "masselot_main_agnostic", CITY,
                 "tables", "uncertainty_runs", RUN)

def rd(name):
    return pd.read_csv(os.path.join(D, f"{name}_{CITY}_improved_fast.csv"))
...
for ax, (name, col, title, color) in zip([e, f, g], [
        ("sens_aai_agg", "aai_agg", "(e) PAWN: annual mortality", "#666666"),
        ("sens_cba_ews", "ews_net_avoided_deaths_25y_cum", "(f) PAWN: EWS net avoided deaths", EWS),
        ("sens_vulnerability", "vuln_2050_svi_p90_p10_gap", "(g) PAWN: 2050 SVI P90-P10 gap", "#7a4fa3")]):
    params, vals = pawn_top(name, col)
...
print(f"(a) {CITY.title()} P5/P95 ratios: ×{p5:.2f} / ×{p95:.2f}"
      f"   (caption currently says ×0.63/×1.62 — update if different)")
